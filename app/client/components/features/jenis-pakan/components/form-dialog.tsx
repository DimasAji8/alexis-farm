...
export function JenisPakanFormDialog({ open, onOpenChange, onSubmit, isLoading, data }: Props) {
  ...
  const { register, handleSubmit, reset, control, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { kode: "", nama: "", satuan: "KG", keterangan: "", isActive: true },
  });

  useEffect(() => {
    if (open) {
      reset(data ? { kode: data.kode, nama: data.nama, satuan: data.satuan, keterangan: data.keterangan || "", isActive: data.isActive } : { kode: "", nama: "", satuan: "KG", keterangan: "", isActive: true });
    }
  }, [data, reset, open]);
